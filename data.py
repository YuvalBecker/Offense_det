...
def read_test_file_all(tokenizer, truncate=512, sentence_transformer=''):
    ...
    lens = get_lens(token_ids)
    token_ids = np.array(pad_sents(token_ids, tokenizer.pad_token_id))
    sentence_embedding = sentence_transformer.encode(tweets)

    return ids, token_ids, lens, mask, label_a, label_b, label_c, sentence_embedding
...
def all_tasks(filepath: str, tokenizer, truncate=512, sentence_transformer=''):
